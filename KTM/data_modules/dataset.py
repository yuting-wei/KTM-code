...
class ClassificationDataset(Dataset):
    def __init__(self, config, vocab, stage='TRAIN', on_memory=True, corpus_lines=None, mode="TRAIN"):
        ...
        self.max_input_length = self.config.kbmt.max_length
    ...
    def __getitem__(self, index):
        ...
        :return: sample -> Dict{'token': List[Str], 'label': List[Str], 'token_len': int, 'mention_senti': List[int], 'mention_theme': List[int]}
        """
    ...
    def _preprocess_sample(self, sample_str):
        ...
        :param sample_str: Str format of json data, "Dict{'token': List[Str], 'label': List[Str] , 'mention': List[Str] }"
        :return: sample -> Dict{'token': List[int], 'label': List[int], 'token_len': int, 'mention_senti': List[int], 'mention_theme': List[int]}
        """
        raw_sample = json.loads(sample_str)
        sample = {'token': [], 'label': [], 'mention_senti': [], 'mention_theme': []}
