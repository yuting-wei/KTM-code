...
class GRU(nn.Module):
    def __init__(self,
        ...
        """
        GRU module
        :param layers: int, the number of layers, config.kbmt.RNN.num_layers
        :param input_dim: int, config.embedding.token.dimension
        :param output_dim: int, config.kbmt.RNN.hidden_dimension
        :param bias: None
        :param batch_first: True
        :param dropout: p = dropout, config.kbmt.RNN.dropout
        :param bidirectional: Boolean , default True, config.kbmt.RNN.bidirectional
        """
...
class KBMT(nn.Module):
    def __init__(self, config):
        """
        TextRCNN
        :param config: helper.configure, Configure Object
        """
        super(KBMT, self).__init__()
        self.config = config
        self.layers_num = config.kbmt.att.layers_num
        self.rnn = GRU(
            layers=config.kbmt.RNN.num_layers,
            input_dim=config.embedding.token.dimension,
            output_dim=config.kbmt.RNN.hidden_dimension,
            batch_first=True,
            bidirectional=config.kbmt.RNN.bidirectional
        )
        hidden_dimension = config.kbmt.RNN.hidden_dimension
        if config.kbmt.RNN.bidirectional:
            hidden_dimension *= 2
        self.rnn_dropout = torch.nn.Dropout(p=config.kbmt.RNN.dropout)

        self.self_attn = nn.ModuleList([
            MultiHeadedAttention(
                hidden_dimension, heads_num=config.kbmt.att.heads_num, dropout=config.kbmt.att.dropout 
            )
            for _ in range(self.layers_num)
        ])

        self.kernel_sizes = config.kbmt.CNN.kernel_size
        self.convs = torch.nn.ModuleList()
        for kernel_size in self.kernel_sizes:
            self.convs.append(torch.nn.Conv1d(
                hidden_dimension,
                config.kbmt.CNN.num_kernel,
                kernel_size,
                padding=kernel_size // 2
                )
            )
        self.top_k = config.kbmt.topK_max_pooling
    ...
    def forward(self, inputs, seq_lens, theme_matrix, senti_matrix):
        ...
        hidden = self.rnn_dropout(text_output) # (batch, max_batch_seq_len, hidden_dimension*2)
        ...
        for i in range(self.layers_num):
            hidden1 = self.self_attn[i](hidden1, hidden1, hidden1, theme_matrix)
        theme_att_emb = hidden1
        for i in range(self.layers_num):
            hidden2 = self.self_attn[i](hidden2, hidden2, hidden2, senti_matrix)
