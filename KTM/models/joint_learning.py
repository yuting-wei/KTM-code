...
class JL(nn.Module):
    def __init__(self, config, label_map, device):
        ...
        # linear transform   
        self.transformation = nn.Linear(2*config.model.linear_transformation.text_dimension,
                                        12 * config.model.linear_transformation.node_dimension)

        # classifier
        self.linear_theme = nn.Linear(10 * config.embedding.label.dimension,
                                10)
        self.linear = nn.Linear(12 * config.embedding.label.dimension,
                                12)
    ...
    def forward(self, theme_feature, senti_feature):
        ...
        senti_feature = torch.cat((senti_feature, theme_feature), 1)
        ...
        # emotion predict
        senti_feature = self.transformation_dropout(self.transformation(senti_feature))
        senti_feature = senti_feature.view(senti_feature.shape[0],
                                         12,
                                         self.config.model.linear_transformation.node_dimension)
        senti_logits = self.dropout(self.linear(senti_feature.view(senti_feature.shape[0], -1)))
