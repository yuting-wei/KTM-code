...
def train(config):
    ...
    # build up model
    ktm = KTM(config, corpus_vocab, model_mode='TRAIN')
    ktm.to(config.train.device_setting.device)

    # define training objective & optimizer
    optimize = set_optimizer(config, ktm)

    # get epoch trainer
    trainer = Trainer(model=ktm,
                      optimizer=optimize,
                      vocab=corpus_vocab,
                      config=config)
    ...
    if os.path.isfile(best_epoch_model_file):
        load_checkpoint(best_epoch_model_file, model=ktm,
                        config=config,
                        optimizer=optimize)
